...
func (r *NotesyncListPostgres) Delete(userId, listId int) error {
...
func (r *NotesyncListPostgres) Update(userId, listId int, input notesync.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id AND ul.list_id=$%d AND ul.user_id=$%d",
		notesyncListsTable, setQuery, usersListsTable, argId, argId+1)
